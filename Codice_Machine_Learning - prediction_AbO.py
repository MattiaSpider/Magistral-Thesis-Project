...
n_device_initials = [['MEPEAI-5mML-20T',
                      'MEPEAI-5mML-100T',
                      'MEPEAI-10mML-20T',
                      'MEPEAI-10mML-100T',
                      'MEPEAI-15mML-20T',
...
for device_initials in n_device_initials:
    ...
    # Perform predictions on n-esimo catione
    Y_Voc_test_LR = lr_regressor.predict(X_test_stand)
    ...
    from sklearn import metrics
    lr_score = np.sqrt(metrics.mean_squared_error(Y_Voc_test, Y_Voc_test_LR))
    knn_score = np.sqrt(metrics.mean_squared_error(Y_Voc_test, Y_Voc_test_KNN))
    rf_score = np.sqrt(metrics.mean_squared_error(Y_Voc_test, Y_Voc_test_RF))
    gb_score = np.sqrt(metrics.mean_squared_error(Y_Voc_test, Y_Voc_test_GB))
    svr_score = np.sqrt(metrics.mean_squared_error(Y_Voc_test, Y_Voc_test_SVR))
    nn_score = np.sqrt(metrics.mean_squared_error(Y_Voc_test, Y_Voc_test_NN))
    
    lr_std = np.std(Y_Voc_test - Y_Voc_test_LR)
    knn_std = np.std(Y_Voc_test - Y_Voc_test_KNN)
    rf_std = np.std(Y_Voc_test - Y_Voc_test_RF)
    gb_std = np.std(Y_Voc_test - Y_Voc_test_GB)
    svr_std = np.std(Y_Voc_test - Y_Voc_test_SVR)
    nn_std = np.std(Y_Voc_test - Y_Voc_test_NN)
    ...
    data = [Y_Voc_test, Y_Voc_test_LR, Y_Voc_test_KNN, Y_Voc_test_RF, Y_Voc_test_GB, Y_Voc_test_SVR, Y_Voc_test_NN]  # Lista di array dei dati per i diversi modelli
...
fig,ax = plt.subplots(figsize=(8,6))
print('All but one validation scores for LR, KNN, RF, GB, SVR, NN,: ' , ABO_scores)
# print('Std. dev: ',stds)
fontsize = 12
plt.rc('xtick', labelsize=fontsize)
plt.rc('ytick', labelsize=fontsize)
model = ['Linear Regression', 'K Nearest Neighbors', 'Random Forest', 'Gradient Boosting', 'SVR', 'Neural Network']
# ax.bar(model,scores,yerr=stds,alpha=0.7, capsize=2)
ax.bar(model, ABO_scores, yerr=ABO_std, alpha=0.7, capsize=2)
ax.set_ylabel('RMSE (mV)',fontsize=fontsize)
ax.set_title('Scores validazione All but One',fontsize=fontsize)
plt.xticks(rotation=90)
plt.ylim(0,100)
plt.show()
plt.rcParams['font.family']="Arial"
